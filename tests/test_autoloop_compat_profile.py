...
class AutoloopCompatProfileTests(TestCase):
    def setUp(self) -> None:
        self.mod = _load_module()

    def test_compat_profile_reaches_target_without_upgrade_target(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            proposal = Dev3Proposal(
                ...
                goal="Improve U01",
                rationale="x",
                edits=[ProposalEdit(path="README.md", operation="append", new_text="\n#x\n")],
                expected_effect="x",
                risk_notes=[],
            ...
            with patch.object(self.mod.Dev3Strategist, "propose", return_value=proposal), patch.object(
                self.mod.Dev4Reviewer, "review", return_value=Dev4Review(verdict="rejected", reasons=["path_not_allowed"])
            ), patch.object(
                ...
                return_value=Dev2ExecutionResult(applied=True, files_changed=["README.md"], patch_handle=None),
            ), patch.object(
                self.mod,
                "run_quality_checks",
