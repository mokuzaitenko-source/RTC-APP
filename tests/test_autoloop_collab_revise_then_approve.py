...
class AutoloopCollabReviseThenApproveTests(TestCase):
    def setUp(self) -> None:
    ...
    def test_revise_then_approve_counts_revision(self) -> None:
        ...
        with patch.object(self.mod.Dev3Strategist, "propose", return_value=proposal), patch.object(
            self.mod.Dev4Reviewer,
            "review",
            side_effect=[Dev4Review(verdict="revise", reasons=["needs_changes"]), Dev4Review(verdict="approved", reasons=["ok"])],
        ), patch.object(
            ...
            return_value=Dev2ExecutionResult(applied=True, files_changed=["tests/test_x.py"], patch_handle=None),
        ), patch.object(
            self.mod,
            "run_quality_checks",
        ...
        self.assertEqual(summary["revised_cycles"], 1)
